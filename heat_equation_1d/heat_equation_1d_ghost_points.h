...
#ifndef HEAT_EQUATION_1D_GHOST_POINTS_H
#define HEAT_EQUATION_1D_GHOST_POINTS_H

#include "../common_defs.h"

// In this version, Neumann BCs are implemented using additional ghost points
...
public:
        ...
        , dt{init_dt}
    {
        ...
        for (long i = 0; i < static_cast<long>(u0.size()); i ++) {
    ...
    std::span<const double> do_step() {
        ...
        for (long i = 0; i < static_cast<long>(u.size()); i ++) {
            ...
            if constexpr (BC_L::bc_type == DIRICHLET)
                if (m == 0)
                    continue;

            if constexpr (BC_R::bc_type == DIRICHLET)
                if (m == M)
                    continue;

            // To manipulate array data, use index i which is equivalent to theoretical index m
            u_new[i] = u[i] + nu * dt / h / h * (u[i+1] - 2.0 * u[i] + u[i-1]);
        }
        ...
        return get_truncated_buffer(u);
    }

    void apply_bcs(GridPoints & buffer) const {
        if constexpr (BC_L::bc_type == NEUMANN)
            // This is a Neumann BC
            buffer[0] = buffer[2]  - 2 * h * BC_L::evaluate(t);
        else
        ...
        if constexpr (BC_R::bc_type == NEUMANN)
            // This is a Neumann BC
            buffer[buffer.size() - 1] = buffer[buffer.size() - 3] + 2 * h * BC_R::evaluate(t);
        else
            // This is a Dirichlet BC
    ...
    [[nodiscard]]
    long array_to_real_index(long i) const {
        if constexpr (BC_L::bc_type == NEUMANN)
            i --;
        return i;
    }

    static std::span<const double> get_truncated_buffer(GridPoints const & buffer) {
        auto begin_itr = buffer.begin();
        if constexpr (BC_L::bc_type == NEUMANN)
            begin_itr ++;

        auto end_itr = buffer.end();
        if constexpr(BC_R::bc_type == NEUMANN)
            end_itr --;

        return std::views::counted(begin_itr, end_itr - begin_itr);
    }
...
#endif //HEAT_EQUATION_1D_GHOST_POINTS_H
